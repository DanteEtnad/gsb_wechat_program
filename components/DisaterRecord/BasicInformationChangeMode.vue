			...
			<uni-row >
				...
				<uni-col :span="16">
					...
					<!-- 	<uni-easyinput
						...
						<uni-data-select  v-model="disaster.name" :localdata="members" :placeholder="disaster.name" @change="NameText"/>
					</view>
				</uni-col>
			</uni-row>
			<!-- 联系电话 -->
		...
		methods: {
			...
			Typechoice(index) {
				...
				if(this.ID=1){
					...
					this.list[this.Index].unselected=true
					this.ID=0
				}
			...
			Scalechoice(index) {
				...
				if(this.ID1=1){
					this.list1[this.Index1].selected1=false
					this.list1[this.Index1].unselected1=true
					this.ID1=0
				}
			...
			Levelchoice(index) {
				...
				if(this.ID2=1){
					this.list3[this.Index2].selected3=false
					this.list3[this.Index2].unselected3=true
					this.ID2=0
				}
				//当再次被选中时，取消当前选中项
