<template>
	<view>
		<view style="padding: 5px;background-color: white;">
			<uni-forms :model="queryForm" ref="queryForm">
				<uni-row>
					...
					<uni-col :span="6">
						<button style="margin-left: 5px;" @click="getAlertData">
							<text>搜索</text>
		...
		<view class="alert-item-container" v-for="item in alertData" :key="item">
...
<script>
	...
	export default {
		...
		methods: {
			openCreateDialog(){
				uni.navigateTo({
					url:'/pages/alertManagement/AlertCreateDialog'
				})
			},
			getAlertData(){
				request({
