						...
						<uni-collapse-item title="手风琴效果" v-for="(item,index) in createData" :key="index">
							...
							<view class="collapse-item-container">
								<view class="collapse-item-main">
									<view class="collapse-item-main-container"
										@click="deleteMember(item.level,textIndex)"
										v-for="(text,textIndex) in item.location" :key="textIndex"
										v-show="item.location.length!==0">
										{{text}}
									</view>
								</view>
		...
		<uni-popup ref="popup" type="bottom" background-color="#fff">
			<view class="popup-container">
				...
				<view class="popup-add">
					<uni-data-picker placeholder="请选择地区" popup-title="请选择地区" v-model="selectedLocation"
						:localdata="locationDataTree" />
...
<script>
	import {
		getAreaOptionsMixins,
		dataCodeTransformMixins
		...
		methods: {
			...
			submit() {
				try{
					...
					if(createForm.alertStartTime>createForm.alertEndTime){
						throw new Error('时间范围异常')
					}
					createForm.alertStartTime = createForm.alertStartTime.split('-').join('').split(':').join('').split(' ')
						.join('')
			...
			imgGenerate() {
				try{
					...
					let mapForm = {
						...
						effectiveStartTime: this.alertForm.alertStartTime.split('-').join('').split(':').join('').split(
							' ').join(''),
						effectiveEndTime: this.alertForm.alertEndTime.split('-').join('').split(':').join('').split(' ')
							.join(''),
						alertAreaLevel: [],
						mode: 0
					}
